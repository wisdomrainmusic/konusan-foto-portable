...
def _lazy_face_enhancer():
    """
    Enhancer=none iken gfpgan/basicsr import zincirine hiç girmeyelim.
    Enhancer istenirse o anda import edelim.
    """
...
class AnimateFromCoeff():
    ...
    def load_cpk_facevid2vid(
        self, checkpoint_path, generator=None, discriminator=None,
    ...
    def load_cpk_mapping(
        self, checkpoint_path, mapping=None, discriminator=None,
    ...
    def generate(self, x, video_save_dir, pic_path, crop_info, enhancer=None, background_enhancer=None, preprocess='crop'):
        ...
        # generated video is 256x256, keep aspect ratio based on crop_info original_size
        original_size = crop_info[0]
        if original_size:
            result = [
                cv2.resize(r, (256, int(256.0 * original_size[1] / original_size[0])))
                for r in result
            ]
        ...
        # ✅ portable: mp3 zorlaması yerine genel from_file kullan
        start_time = 0
        try:
        ...
        if preprocess.lower() == 'full':
            ...
            # ✅ enhancer sadece istendiyse import + çalıştır
            if enhancer:
                face_enhancer = _lazy_face_enhancer()
